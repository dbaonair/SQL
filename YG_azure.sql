...
JOIN sys.query_store_query qsq ON
    qsq.query_text_id = qsqt.query_text_id
JOIN sys.query_store_plan qsp ON
    qsq.query_id = qsp.query_id
    WHERE qsq.query_hash = 0x8FA809F150DA1A63
AND qsp.last_execution_time BETWEEN '11/15/2017 4:30:00 PM' AND '11/15/2017 6:30:00 PM'
...
select object_name(i.id) as OBJECTNAME,i.indid,i.name,i.groupid, schema_name(uid),
--    f.name as FGNAME,
	i.dpages,i.reserved
	  ,case when indid=1   then (i.dpages/1024.0/1024.0)*8  else (i.reserved/1024.0/1024.0)*8   end as SPACEUSED 
		,i.rows  
--into #tmp1
		from sys.sysindexes i
		inner join sys.sysobjects o on i.id=o.id 
		left join sys.filegroups f on  f.data_space_id = i.groupid
		left join sys.data_spaces s on f.data_space_id = s.data_space_id 
...
ADD EVENT sqlserver.sql_batch_completed(
    ACTION(sqlserver.client_app_name,sqlserver.database_id,sqlserver.query_hash,sqlserver.session_id,sqlserver.sql_text,sqlserver.username)
    WHERE ((([package0].[divides_by_uint64]([sqlserver].[session_id],(5))) AND ([package0].[greater_than_uint64]([sqlserver].[database_id],(4)))) AND ([sqlserver].[username]=N'nirvana_appuser')))
ADD TARGET package0.asynchronous_file_target(SET filename=N'https://dbsexportimport01.blob.core.windows.net/exportfolder/FileName.xel'),
...
ADD EVENT sqlserver.sql_statement_completed(
    ...
    WHERE ([package0].[greater_than_uint64]([sqlserver].[database_id],(4)) AND [package0].[equal_boolean]([sqlserver].[is_system],(0)) AND [duration]>(100000)))
ADD TARGET package0.asynchronous_file_target(SET filename=N'https://dbsexportimport01.blob.core.windows.net/exportfolder/Prod_Falcon_lrq.xel'),
...
CREATE EVENT SESSION [Prod_Hazelcastuser] ON DATABASE 
ADD EVENT sqlserver.error_reported(
    ...
    WHERE ([package0].[greater_than_uint64]([sqlserver].[database_id],(4)) AND [package0].[equal_boolean]([sqlserver].[is_system],(0)) AND [sqlserver].[username]=N'Hazelcast_user')),
ADD EVENT sqlserver.rpc_completed(
    ACTION(sqlserver.client_app_name,sqlserver.database_id,sqlserver.database_name,sqlserver.query_hash,sqlserver.session_id,sqlserver.sql_text,sqlserver.username)
    WHERE ([package0].[greater_than_uint64]([sqlserver].[database_id],(4)) AND [package0].[equal_boolean]([sqlserver].[is_system],(0)) AND [sqlserver].[username]=N'Hazelcast_user')),
ADD EVENT sqlserver.sql_batch_completed(
    ACTION(sqlserver.client_app_name,sqlserver.database_id,sqlserver.database_name,sqlserver.query_hash,sqlserver.session_id,sqlserver.sql_text,sqlserver.username)
    WHERE ([package0].[greater_than_uint64]([sqlserver].[database_id],(4)) AND [package0].[equal_boolean]([sqlserver].[is_system],(0)) AND [sqlserver].[username]=N'Hazelcast_user'))
ADD TARGET package0.asynchronous_file_target(SET filename=N'https://dbsexportimport01.blob.core.windows.net/exportfolder/preprod_appuser.xel')
WITH (MAX_MEMORY=4096 KB,EVENT_RETENTION_MODE=ALLOW_SINGLE_EVENT_LOSS,MAX_DISPATCH_LATENCY=30 SECONDS,MAX_EVENT_SIZE=0 KB,MEMORY_PARTITION_MODE=NONE,TRACK_CAUSALITY=ON,STARTUP_STATE=OFF)
GO
...
ADD TARGET package0.asynchronous_file_target(SET filename=N'https://dbsexportimport01.blob.core.windows.net/exportfolder/PRod_nirvana_Loginlogout.xel'),
...
ADD TARGET package0.asynchronous_file_target(SET filename=N'https://dbsexportimport01.blob.core.windows.net/exportfolder/error_report.xel'),
ADD TARGET package0.ring_buffer(SET max_memory=(4096))
WITH (MAX_MEMORY=4096 KB,EVENT_RETENTION_MODE=ALLOW_SINGLE_EVENT_LOSS,MAX_DISPATCH_LATENCY=30 SECONDS,MAX_EVENT_SIZE=0 KB,MEMORY_PARTITION_MODE=NONE,TRACK_CAUSALITY=OFF,STARTUP_STATE=OFF)


https://pozfunction.azurewebsites.net/api/ScaleSqlDatabase?code=1KA2PIpXnUX9lLO93wvj6HVs/p3TxwqJmD25Q5XuvkG5j4Mm64sQXA==&serverName=pozbltt01&databaseName=Nirvana&scaleUp=1
https://pozfunction.azurewebsites.net/api/ScaleSqlDatabase?code=1KA2PIpXnUX9lLO93wvj6HVs/p3TxwqJmD25Q5XuvkG5j4Mm64sQXA==&serverName=pozbltt01&databaseName=Nirvana&scaleUp=1
https://pozfunction.azurewebsites.net/api/ScaleDatabase?code=B9SU3SEmla7L9zIshC6rplxiDUNnDbf8bpzv40fK6RVMXOLLp0Gikw==&serverName=pozbltt01&databaseName=Nirvana&scaleUp=1
